...
class MySpecialAdsVC: UIViewController {
    ...
    public func setNavigationControll(){
        ...
        navigationItem.title = "إعلاناتي المميزة"
    ...
    func getData(){
        ...
        API.GETwithHeader(url: URLs.GetMyAds, header: header) { (success, value) in
            if success{
                ...
                let dict = value
                if let posts = dict["feature"] as? [[String:Any]]{
                    for p in posts{
                        let id = p["ID"] as? Int
...
extension MySpecialAdsVC : UITableViewDelegate, UITableViewDataSource{
